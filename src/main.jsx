...
import { ChevronDown, ExternalLink, Heart, Menu, Play, Search, Sparkles, X } from 'lucide-react'
import './styles.css'
...
function App(){
  const [menu,setMenu]=useState(false), [category,setCategory]=useState('Toutes'), [query,setQuery]=useState(''), [openFaq,setOpenFaq]=useState(0)
  ...
  return <div>
    <header className="header"><div className="nav-wrap">
      ...
      <nav className={menu?'nav open':'nav'}>{[['accueil','Accueil'],['videos','Vidéos'],['apropos','À propos'],['stats','Statistiques'],['faq','FAQ']].map(([id,l])=><button key={id} onClick={()=>go(id)}>{l}</button>)}</nav>
    ...
    <main>
      ...
      <section id="videos" className="section videos"><div className="section-head"><div><span className="kicker">LA VIDÉOTHÈQUE</span><h2>Explorer les leçons</h2><p>Une bibliothèque de concepts expliqués par l’image, classés par thème.</p></div><label className="search"><Search size={18}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Rechercher une vidéo"/></label></div>
        ...
        <div className="video-grid">{filtered.map(v=><article className="video-card" key={v.id}><a className="thumb" href={`https://www.youtube.com/watch?v=${v.id}`} target="_blank"><img src={`https://img.youtube.com/vi/${v.id}/hqdefault.jpg`} alt=""/><span className="play"><Play fill="currentColor"/></span><small>{v.duration}</small></a><div className="video-body"><div className="meta">{v.category} · {v.date}</div><h3>{v.title}</h3><p>{v.description}</p><footer><span>{v.views}</span><ExternalLink size={17}/></footer></div></article>)}</div>
    ...
    <footer className="footer"><div className="brand inverse"><span className="brand-mark">K</span><span><strong>KobiPy</strong><small>Mathématiques en mouvement</small></span></div><div><a href={CHANNEL_URL} target="_blank">YouTube</a><a href={TIPEEE_URL} target="_blank">Tipeee</a><button onClick={()=>go('faq')}>FAQ</button></div><small>Prototype éditorial — données publiques indicatives</small></footer>
  </div>
}
